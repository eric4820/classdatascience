...
ggplot(YEAR, aes(x=as.factor(month), total, fill = as.factor(year)))+
  geom_bar(stat = "identity", position = "dodge")+
  labs(title = "days of the month performance",x="month",y="total")+
...
#NO OF MALLS IN A LOCATION using a  bar graph
...
#location has the least no of malls.
  ...
  ggplot(YEAR ,aes(x=month, y=total)) +
  geom_line() +
  geom_point() +
  scale_color_viridis_d() +
  ggtitle("PAYMENT PERFORMANCE") +
    labs(x="month",y="TOTAL MONEY")+
  theme_ipsum() +
...
library(tidyverse)
View(infert)
y <- ggplot(PERFORMANCE_PER_MONTHS_OF_THE_YEAR,aes(reorder(x=as.factor(month),total) ,y=total))+
...
nsum_plot <-ggplot(new,aes(x=education,y=count ))+
  geom_bar(stat = "identity", position = "dodge", fill = "green")+
  facet_wrap(~case)+
  #geom_text(aes(label=paste0(round(percentage ,0),"%") ,vjust=-.5 ,size=2))+
  labs(title = "distribution of education level",x="Education level",y="Percentages")+
  theme_classic()
  #scale_fill_manual(values = c("green","light green"))
nsum_plot
sum_plot <-ggplot(Edu_level,aes(x=education,y=percentage,fill=as.factor(case)))+
  geom_bar(stat = "identity", position = "dodge")+
  geom_text(aes(label=paste0(round(percentage ,0),"%") ,vjust=-.5 ,size=2))+
  labs(title = "distribution of education level",x="Education level",y="Percentages")+
  theme_classic()+
